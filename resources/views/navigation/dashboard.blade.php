<div class="menu-list">
  <ul id="menu-content" class="menu-content collapse out">
    <li data-toggle="collapse" data-target="#home" class="collapsed active">
      <a href="#">
        <i class="fa fa-home fa-lg"></i>  Home</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="home">
      <li><a href="#">Profile</a></li>
      <li><a href="#">Messages</a></li>
      <li><a href="#">Task</a></li>
      <li><a href="#">Notes</a></li>
      <li><a href="#">Account Settings</a></li>
      <li><a href="#">Logout</a></li>
    </ul>
    <li data-toggle="collapse" data-target="#overview" class="collapsed active">
      <a href="#">
        <i class="fa fa-briefcase fa-lg"></i>  Overview</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="overview">
      <li><a href="#">Company</a></li>
      <li><a href="#">Policies</a></li>
      <li><a href="#">Accounts</a></li>
      <li><a href="#">Audit Trail</a></li>
      <li><a href="#">System Setting</a></li>
    </ul>
    <li data-toggle="collapse" data-target="#sales" class="collapsed active">
      <a href="#">
        <i class="fa fa-shopping-cart fa-lg"></i>  Sales</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="sales">
      <li><a href="#">Restaurant Mode</a></li>
      <li><a href="#">Retailer Mode</a></li>
      <li><a href="#">Delivery Mode</a></li>
    </ul>

    <li data-toggle="collapse" data-target="#products" class="collapsed active">
      <a href="#">
        <i class="fa fa-list fa-lg"></i>  Inventory</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="products">
      <li><a href="items">Products</a></li>
      <li><a href="#">Customers</a></li>
      <li><a href="#">Suppliers</a></li>
      <li><a href="#">Categories</a></li>
      <li><a href="#">Archives</a></li>
    </ul>
    <li data-toggle="collapse" data-target="#reports" class="collapsed active">
      <a href="#">
        <i class="fa fa-clipboard fa-lg"></i>  Reports</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="reports">
      <li><a href="#">Documents</a></li>
      <li><a href="#">Expenditures</a></li>
      <li><a href="#">Invoices</a></li>
    </ul>
    <li data-toggle="collapse" data-target="#analytics" class="collapsed active">
      <a href="#">
        <i class="fa fa-bar-chart fa-lg"></i>  Analytics</span> <span class="arrow">
      </a>
    </li>
    <ul class="sub-menu collapse" id="analytics">
      <li><a href="#">Supply Purchase Request</a></li>
      <li><a href="#">Supply Acquisition</a></li>
      <li><a href="#">Supply Status</a></li>
      <li><a href="#">Depreciation / Expiration</a></li>
    </ul>
  </ul>
</div>
